...
SUPERVISOR_SYSTEM_PROMPT = (
    ...
    "1. RAG (unstructured documents): Contains uploaded PDFs, CSV spreadsheets, product manuals, market analysis, and company policy documents. NOTE: Any uploaded CSV files (like sales spreadsheets) are indexed in the RAG vector database, NOT as tables in the SQL database.\n"
    ...
    "- 'sql': Use ONLY when the question refers strictly to columns and tables in the SQL Database Schema below. If the query requests fields or attributes (e.g., 'gender', 'rating', 'tax', 'cogs', 'payment method', 'unit price', 'quantity', 'total', etc.) that are NOT defined in the columns of the SQL Schema context, do NOT route to 'sql' (even if the query contains general words like 'customers' or 'sales').\n"
    "- 'rag': Use when the question is about uploaded files, spreadsheets, or PDFs. If the query requests fields or attributes (e.g., 'gender', 'rating', 'tax', 'cogs', 'payment method', 'unit price', 'quantity', 'total', etc.) that are NOT present in the SQL Database Schema but are likely contained in your uploaded spreadsheets (like a supermarket sales dataset), or if the active session is analyzing a spreadsheet, you MUST route to 'rag'. If the query refers to a filename in the 'Available Documents' list (e.g., 'SuperMarket Analysis.csv' or sales CSV data), you MUST route to 'rag'.\n"
    "- 'hybrid': Use when answering requires joining metrics from the SQL database schema with the uploaded documents.\n"
